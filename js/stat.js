...
var HISTOGRAM_START_Y = 235; // (270-150)/2=60 высота пространства над и под гистограммой; 280-150+60-25(текст и отступ)
var HISTOGRAM_TEXT_Y = 255; // 280-15(высота текста)-10(отступ)
var HISTOGRAM_TEXT_GAP = 30;
var columnColor;
...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    ctx.fillRect(HISTOGRAM_START_X + (HISTOGRAM_COLUMN_GAP + HISTOGRAM_WIDTH) * i, HISTOGRAM_START_Y, HISTOGRAM_WIDTH, (HISTOGRAM_HEIGHT * times[i]) / maxTime * -1);

    ctx.fillStyle = COLOR_BLACK;
    ctx.fillText(time, HISTOGRAM_START_X + (HISTOGRAM_COLUMN_GAP + HISTOGRAM_WIDTH) * i, HISTOGRAM_TEXT_Y - (HISTOGRAM_HEIGHT * times[i]) / maxTime - HISTOGRAM_TEXT_GAP);
